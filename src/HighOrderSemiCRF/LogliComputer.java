...
public class LogliComputer implements Schedulable {
    ...
    public double[][][] computeMarginal(DataSequence seq, double[][] logAlpha, double[][] logBeta, double logZx) {
        double[][][] marginal = new double[seq.length()][seq.length()][featureGen.patternMap.size()];
        for (int zID = 0; zID < featureGen.patternMap.size(); zID++) {
            ...
            for (int segStart = 0; segStart < seq.length(); segStart++) {
                for (int segEnd = segStart; segEnd < seq.length() && segEnd - segStart < maxmem; segEnd++) {
                    marginal[segStart][segEnd][zID] = Double.NEGATIVE_INFINITY;
					...
					for (int i = 0; i < featureGen.patternTransition1[zID].size(); i++) {
						...
						marginal[segStart][segEnd][zID] = Utility.logSumExp(marginal[segStart][segEnd][zID], logAlpha[BASE + segStart - 1][piID] + logBeta[segEnd + 1][piyID] + featuresScore);
					}
					
					marginal[segStart][segEnd][zID] = Math.exp(marginal[segStart][segEnd][zID] - logZx);
                }
            }
        }
        return marginal;
        ...
        for (int zID = 0; zID < featureGen.patternMap.size(); zID++) {
            ...
            for (int segStart = 0; segStart < seq.length(); segStart++) {
                for (int segEnd = segStart; segEnd < seq.length() && segEnd - segStart < maxmem; segEnd++) {
                    ...
                    for (int index : feats) {
						Feature feat = featureGen.featureList.get(index);
                        expectation[index] += feat.value * marginal[segStart][segEnd][zID];
                    }
                }
            }
        }
