...
public class SentenceObsGenerator implements Schedulable {
    ...
    public Object compute(int taskID) {
		...
		seq.observationMap = new ArrayList[seq.length()][seq.length()];
        
        for (int segStart = 0; segStart < seq.length(); segStart++) {
            for (int segEnd = segStart; segEnd < seq.length() && segEnd - segStart < featGen.params.maxSegment; segEnd++) {
                ...
                seq.observationMap[segStart][segEnd] = obsIndices;
            }
        }
        return seq;
    }
